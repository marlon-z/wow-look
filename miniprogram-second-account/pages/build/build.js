...
Page({
  ...
  onShareAppMessage: function (options) {
    ...
    if (ds.shareType === 'build' && classKey) {
      return {
        title: '看看这套' + className + specName + '配装｜艾泽拉斯装备搭配',
    ...
    // 默认(右上…菜单转发)
    if (classKey) {
      return { title: className + specName + ' 配装 · 艾泽拉斯装备搭配', path: '/pages/build/build?' + this.buildShareBase() };
    }
    return { title: '艾泽拉斯装备搭配 · 配装模拟器', path: '/pages/build/build' };
  },

  onShareTimeline: function () {
    if (!this.data.selectedClassKey) {
      return { title: '艾泽拉斯装备搭配 · 配装模拟器' };
    }
    return {
      title: (this.data.selectedClassName || '') + (this.data.selectedSpecName || '') + ' 配装 · 艾泽拉斯装备搭配',
